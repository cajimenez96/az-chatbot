...
export default function BuilderPage() {
  ...
  const [livePreviewBlock, setLivePreviewBlock] = useState<Partial<IBlock> | null>(null);
  ...
  const filteredBlocks = (blocks ?? []).filter(
    (b) => b && b.id && (activeTab === "faq" ? b.isFaq : !b.isFaq)
  );
  ...
  const handleCreate = () => {
    setSelectedBlock(null);
    setLivePreviewBlock({ id: "", type: "message", message: "", options: [], isFaq: activeTab === "faq" });
    setIsCreating(true);
  };
  ...
  return (
    <div className="builder-layout bg-canvas relative">
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none bg-[radial-gradient(#000_1px,transparent_1px)] bg-size-[24px_24px]" />
      ...
      <header className="builder-header">
        ...
        >
          ...
          {activeTab === "faq" ? "NUEVA FAQ" : "NUEVO BLOQUE"}
        </Button>
      </header>

      <div className="grid grid-cols-12 gap-12 flex-1 min-h-0 relative z-10 overflow-hidden mb-15!">
        {/* ── Sidebar ───────────────────────────────────────────── */}
        <div className="col-span-3 flex flex-col gap-6 overflow-y-auto pr-4 no-scrollbar">

          {/* Tab switcher */}
          <div className="flex bg-surface-soft rounded-2xl gap-1">
            <button
              onClick={() => handleTabChange("flow")}
              className={cn(
                "flex-1 flex items-center justify-center gap-2 p-2! rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300",
                activeTab === "flow"
            ...
            <button
              ...
              className={cn(
                "flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300",
                activeTab === "faq"
                  ? "bg-primary text-on-primary shadow-sm"
                  : "text-mute hover:text-ink"
              )}
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-[11px] font-black text-ink uppercase tracking-[0.25em] opacity-40 pl-2.5!">
              {activeTab === "flow" ? "BLOQUES DEL FLUJO" : "BASE DE CONOCIMIENTO"}
            </h3>
            <div className="h-px flex-1 mx-4 bg-hairline opacity-50" />
          ...
          <div className="flex flex-col gap-3">
            {loading && filteredBlocks.length === 0 ? (
              <div className="p-10 text-center border border-hairline border-dashed rounded-2xl bg-surface-soft/30 backdrop-blur-sm">
                <p className="text-xs text-mute font-medium italic">Cargando...</p>
              </div>
            ) : filteredBlocks.length === 0 ? (
              <div className="p-10 text-center border border-hairline border-dashed rounded-2xl bg-surface-soft/30 backdrop-blur-sm">
                <p className="text-xs text-mute font-medium italic">
                  {activeTab === "faq" ? "Sin FAQs configuradas aún." : "Sin bloques de flujo."}
                </p>
              </div>
            ) : (
              filteredBlocks.map((block) => {
                ...
                const Icon =
                  block.type === "menu" ? List : block.type === "question" ? HelpCircle : MessageSquare;
                ...
                return (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={cn(
                            ...
                            isSelected ? "bg-black/10 text-ink" : "bg-black/5 text-mute border border-black/5"
                          )}
                        >
                          {block.type}
                        </span>
                    ...
                    {/* Keywords pills para FAQs */}
                    {activeTab === "faq" && block.keywords && block.keywords.length > 0 && (
                      <div className="flex gap-1 mt-2 flex-wrap">
                        {block.keywords.slice(0, 3).map((kw) => (
                          <span
                            key={kw}
                            className={cn(
                              "text-[8px] font-bold px-1.5 py-0.5 rounded-md",
                              isSelected ? "bg-black/10 text-ink" : "bg-surface-soft text-mute"
                            )}
                          >
                            {kw}
                          </span>
          ...
          {selectedBlock || isCreating ? (
            ...
            <div className="flex flex-col items-center justify-center min-h-[600px] bg-white/30 backdrop-blur-md border border-hairline border-dashed rounded-3xl p-16 text-center group hover:bg-white/50 transition-all duration-500">
              ...
              <h3 className="text-2xl font-black text-ink tracking-tight mb-4 uppercase font-heading">
                {activeTab === "faq" ? "Base de Conocimientos" : "Centro de Control"}
              </h3>
              <p className="text-sm text-body/60 max-w-[280px] leading-relaxed font-medium">
                {activeTab === "faq"
                  ? "Seleccioná una FAQ o creá una nueva. El bot la dispara automáticamente cuando detecta las palabras clave."
                  : "Elegí un bloque del flujo conversacional o iniciá uno nuevo desde el botón superior."}
              </p>
          ...
          <div className="phone-mockup">
            <div className="whatsapp-header">
              ...
              <div className="flex-1">
                <p className="text-[13px] font-bold leading-tight">Asistente Virtual</p>
                ...
                >
                  <div className="chat-bubble mt-3! py-2! px-3! ml-2.5!">
                    <p className="whitespace-pre-wrap leading-relaxed">
                      {livePreviewBlock.message || "Escribí un mensaje para previsualizar..."}
                    </p>
                  </div>
